class BestFitMemoryAllocator:
    ...
    def allocate_request(self, process_no, request):
        ...
        best_index = -1
        """ 
        best_index tracks the index of the best-suited memory block. Initialized to -1 (indicating no suitable block found yet).
        """
        for i, (block_size, is_free) in enumerate(self.memory_blocks):
            """  
            Iterates over all memory blocks using enumerate():
            :i: Current index of the block. 
            :block_size: Size of the memory block.
            :is_free: Boolean indicating if the block is free.
            """
            if is_free and block_size >= request:
                """ 
                Checks if the block is free (is_free) and can accommodate the requested size (block_size >= request).
                """
                if best_index == -1 or self.memory_blocks[best_index][0] > block_size:
                    best_index = i
                    """ 
                    Updates best_index:
                    If no suitable block is found yet (best_index == -1), or
        ...
        Calculate remaining_size (block size after allocation).
        Update the block's tuple:
        Set the new size to remaining_size.
        ...
        Return the block number (1-indexed for user clarity).
            If no block is found, return -1 to indicate allocation failure.
            """
    ...
    def display_allocation(self, processes):
        ...
        print(f"{'Process No.':<15}{'Process Size':<15}{'Block No.':<10}")  
        """ 
        Prints a formatted header for the memory allocation table:
        Process No., Process Size, and Block No.
        """
...
if __name__ == "__main__":
    ...
    """
    Creates an instance of BestFitMemoryAllocator with predefined block sizes:
    [200, 300, 400, 100, 150].
    """
    ...
    """ 
    Calls display_allocation() to print
    the allocation results for each process.
    """
    ...
    Process 1: Allocated to Block 5 (120 fits in 150; remaining size is 30).
